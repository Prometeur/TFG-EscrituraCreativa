/*
*  Name_file :SearchApplicantRes.js
*  Description: Contiene una lista de resultado de la búsqueda de los estudiantes que solicitan acceso.
*    
*/
...
const baseUrl = "http://localhost:3001/user/searchApplicant";

const cookies = new Cookies();

class SearchApplicantRes extends Component {
    ...
    changeViewApplicantProfile = (idApplicant) => 
     {
   ...
   componentDidMount() {
    ...
    render() {
            ...
            {this.state.filteredData.map(student => {
                return (
                    <tr>
                        ...
                        <td><button text='Ver Perfil' onClick={() => this.changeViewApplicantProfile(student.id)}>Ver perfil</button></td>
                    </tr>
                )
            })}
        </tbody>
        ...
        return (
            <>
                <h1>Solicitudes de nuevo acceso:</h1>
                <nav>
                    <button onClick={() => this.cerrarSesion()}>Cerrar Sesión</button>
                </nav>
                <div>
                <h2> Resultados de buscar solicitantes con {cookies.get('searchType')} similar a: {cookies.get('searchStudent')}</h2>
                <h1></h1>
                        <label>Buscar solicitudes: </label>
                        <br />
...
export default SearchApplicantRes;