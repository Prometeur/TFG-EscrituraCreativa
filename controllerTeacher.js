...
function editWriting(req, res) {
    ...
    modelTeacher.saveComments(idWriting, commentary,function (err, result) {
        if (err) {
            console.log(err.message);
            res.status(500).send({ error: err.message });
        }
    });
    modelTeacher.editWriting(idWriting, idGroup, idChallenge, idWriter, title,text, score,type,finish, function (err, result) {
        if (err) {
            console.log(err.message);
            res.status(500).send({ error: err.message });
        }
